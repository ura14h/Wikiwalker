...
- シートの本文中の記事リンクをクリックすると、Wikipedia と同じようにその記事へ移れます（リンク先の記事が隣の恒星として置かれます）
...
API への要求は [Wikimedia の利用制限](https://www.mediawiki.org/wiki/Wikimedia_APIs/Rate_limits)に対して余裕を持った頻度に抑え、被リンクは全数ではなく無作為標本だけを取得します。具体的な制御は [docs/design.md](docs/design.md) の「レート制御」を参照してください。
